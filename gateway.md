# 🎓 Developer Portfolio: Gateway

Welcome to my GitHub portfolio repository! I am currently a Computer Information Technology student at Wayne State College. This gateway serves as a central directory to navigate my coursework, highlighting my progress in enterprise mainframe computing and object-oriented software development.

---

## 📁 Table of Contents

| Repository | Primary Tech | Category |
| :--- | :--- | :--- |
| [UTIL2000](#util2000-utility-billing-engine) | COBOL / JCL | CIS352 Intro to Enterprise Computing |
| [CALC2000](#calc2000-future-value-calculator) | COBOL | CIS352 Intro to Enterprise Computing |
| [RPT2000](#rpt2000-year-to-date-sales-report) | COBOL | CIS352 Intro to Enterprise Computing |
| [SEQ3000](#seq3000-employee-file-maintenance) | COBOL | CIS352 Intro to Enterprise Computing |
| [RPT5000](#rpt5000-sales-report-program) | COBOL | CIS352 Intro to Enterprise Computing |
| [RPT6000](#rpt6000-advanced-sales-report) | COBOL | CIS352 Intro to Enterprise Computing |
| [Bank App](#bank-app) | Java | Intro to Programming |
| [Ultimate Calculator](#ultimate-calculator-app) | Java | Intro to Programming |

---

## 📂 Project Summaries

### UTIL2000: Utility Billing Engine
* **Short Summary:** A mainframe utility billing engine that calculates invoices using a three-tier progressive rate scale.
* **Technologies Used:** COBOL, JCL, z/OS Mainframe Environment.
* **Key Learning Concepts:** Progressive calculation logic and batch report formatting.
* **Project Status:** ✅ Completed
* **Course / Self-Project:** CIS352 Introduction to Enterprise Computing
* **Thumbnail Screenshot:** ![UTIL2000](assets/util2000-thumb.png)
* **Repository Link:** 🔗 [View UTIL2000 Repository](#)

[⬆ Back to TOC](#-table-of-contents)

---

### CALC2000: Future Value Calculator
* **Short Summary:** Computes the future value of an investment using predefined input values to demonstrate program flow.
* **Technologies Used:** COBOL, GitHub.
* **Key Learning Concepts:** `COMPUTE` statements, numeric editing, and `PERFORM` logic.
* **Project Status:** ✅ Completed
* **Course / Self-Project:** CIS352 Introduction to Enterprise Computing
* **Thumbnail Screenshot:** ![CALC2000](Screenshots/Screenshot%202026-01-29%20130847.png)
* **Repository Link:** 🔗 [View CALC2000 Repository](#)

[⬆ Back to TOC](#-table-of-contents)

---

### RPT2000: Year-to-Date Sales Report
* **Short Summary:** Processes customer master records to generate a multi-page business report with automated metrics.
* **Technologies Used:** COBOL, JCL.
* **Key Learning Concepts:** Zero-value handling, file-control configuration, and report engineering.
* **Project Status:** ✅ Completed
* **Course / Self-Project:** CIS352 Introduction to Enterprise Computing
* **Thumbnail Screenshot:** ![RPT2000](assets/output.png)
* **Repository Link:** 🔗 [View RPT2000 Repository](#)

[⬆ Back to TOC](#-table-of-contents)

---

### SEQ3000: Employee File Maintenance
* **Short Summary:** Performs sequential file maintenance (Add, Delete, Change) on an employee master file.
* **Technologies Used:** COBOL, JCL.
* **Key Learning Concepts:** Switch-driven logic and HIGH-VALUE end-of-file handling.
* **Project Status:** ✅ Completed
* **Course / Self-Project:** CIS352 Introduction to Enterprise Computing
* **Thumbnail Screenshot:** ![SEQ3000](assets/seq3000-thumb.png)
* **Repository Link:** 🔗 [View SEQ3000 Repository](#)

[⬆ Back to TOC](#-table-of-contents)

---

### RPT5000: Sales Report Program
* **Short Summary:** Generates a Year-To-Date Sales Report highlighting change in sales amount and percentage.
* **Technologies Used:** COBOL, JCL.
* **Key Learning Concepts:** Control break logic and multi-level data aggregation.
* **Project Status:** ✅ Completed
* **Course / Self-Project:** CIS352 Introduction to Enterprise Computing
* **Thumbnail Screenshot:** ![RPT5000](assets/rpt5000.png)
* **Repository Link:** 🔗 [View RPT5000 Repository](#)

[⬆ Back to TOC](#-table-of-contents)

---

### RPT6000: Advanced Sales Report
* **Short Summary:** An enhanced sales reporting program focusing on customer-level data and branch/representative totals.
* **Technologies Used:** COBOL, JCL.
* **Key Learning Concepts:** Accumulator roll-ups and complex PIC clause formatting.
* **Project Status:** ✅ Completed
* **Course / Self-Project:** CIS352 Introduction to Enterprise Computing
* **Thumbnail Screenshot:** ![RPT6000](assets/output.png)
* **Repository Link:** 🔗 [View RPT6000 Repository](#)

[⬆ Back to TOC](#-table-of-contents)

---

### Bank App
* **Short Summary:** A banking interface application developed to manage user accounts, including withdrawals, deposits, and unique ID generation.
* **Technologies Used:** Java, IntelliJ IDEA.
* **Key Learning Concepts:** Object-Oriented Programming (OOP) principles, method implementation, and unique ID generation logic.
* **Project Status:** ✅ Completed
* **Course / Self-Project:** Academic Software Engineering Project
* **Thumbnail Screenshot:** ![Bank App Demo](https://github.com/jascha10/BankApp/blob/master/Assets/bankapp%20(3).gif)
* **Repository Link:** 🔗 [View Bank App Repository](https://github.com/jascha10/BankApp)

[⬆ Back to TOC](#-table-of-contents)

---

### Ultimate Calculator App
* **Short Summary:** A feature-rich calculator that handles basic arithmetic as well as advanced functions like precision control and memory storage.
* **Technologies Used:** Java, IntelliJ IDEA.
* **Key Learning Concepts:** Advanced mathematical functions, precision management, and unit testing via testbenches.
* **Project Status:** ✅ Completed
* **Course / Self-Project:** Academic Software Engineering Project
* **Thumbnail Screenshot:** ![Calculator Demo](assets/calculator-thumb.png)
* **Repository Link:** 🔗 [View Calculator Repository](https://github.com/jascha10/Calculator-App)

[⬆ Back to TOC](#-table-of-contents)
